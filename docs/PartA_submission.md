# PART A-R1: Project description (Updated)
...
4. Give users the option to view/join multiple group pages.
5. Allow users to create group pages that are specific to a particular area.
6. Allow page creators(admin) to delete group(s) that they have created.
7. Allow page creators to regulate and remove posts within the group(s) that they have created.
8. Allow users to search group(s) by postcode.
9. Allow page members to create posts in the group page.
10. Allow page members to comment on posts in the group page.
11. Allow post creators to update and and remove their own posts.

Additional feature(s) include:-
1. Allow user to manage their own accounts such as changing passwords and deleting their accounts (Added 15 Feb) - not implemented in this iteration.
...
![Application Architecture Diagram - original](AAD.png)

Above is the original architecture plan as submitted in Part A that consists of the following components:-
...
![Application Architecture Diagram - updated](AAD_updated.png)
This AAD shows both the application architecture as contained within an EC2 instance as well as the deployment architecture in Amazon Web Services. The application is included with Gunicorn which acts as an application server as a unit within an EC2 instance. This EC2 instance is represented in the deployment architecture diagram as the EC2 logo. As indicated, the database is not included within the same EC2 instance but is placed in a separate private subnet(as illustrated in the deployment architecture diagram).

The Deployment architecture diagram show where each component(as labelled) are placed in the cloud deployment. Arrows show how each component communicate with one another within the deployment infrastructure.
...
![Registration/Login](register_login_pages.png)
...
## Group detail page
![Group details](group_detail_page.png)
...
## Account management page(additional feature) - not implemented
![Account management](account_management.png)
...
![Feb 8](trello_screenshots/210208.png)
